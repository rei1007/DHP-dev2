...
export async function getWhitelist() {
    ...
    if (!supabaseClient) {
        try {
            ...
            console.error('[getWhitelist] Initialization failed');
            return [];
        }
    }
    
    if (!supabaseClient) {
        console.error('[getWhitelist] Client not initialized');
        return [];
    }
...
// Add to Whitelist
export async function addToWhitelist(whitelistData) {
    if (!supabaseClient) {
        await initSupabaseClient();
    }
    if (!supabaseClient) {
    ...
    const dbData = {
        discord_id: whitelistData.discordId,
        name: whitelistData.name,
        role: whitelistData.role || 'admin',
        added_by: whitelistData.addedBy,
        notes: whitelistData.notes
    };
...
export async function deleteFromWhitelist(id) {
    if (!supabaseClient) {
        await initSupabaseClient();
    }
    if (!supabaseClient) {
